...
public struct ListNetworkObjectsTool: PKTool, Sendable {
    ...
    public let toolDescription = "Lists currently advertised Gnostic network objects."
...
public struct InspectNetworkObjectTool: PKTool, Sendable {
    private let service: DiscoveredWorkspaceAttachmentService
    init(service: DiscoveredWorkspaceAttachmentService) { self.service = service }
    public let callName = "inspect_network_object"
    public let name = "Inspect network object"
    public let toolDescription = "Inspects one provider-scoped advertised Gnostic object."
    public let requiresPermission = false
    public let sideEffects: ToolSideEffects = .none
    public var parametersSchema: Schema {
...
public struct AttachWorkspaceTool: PKTool, Sendable {
    private let service: DiscoveredWorkspaceAttachmentService
    init(service: DiscoveredWorkspaceAttachmentService) { self.service = service }
    public let callName = "attach_workspace"
    public let name = "Attach workspace"
    public let toolDescription = "Imports and attaches an unambiguous discovered workspace to a timeline."
