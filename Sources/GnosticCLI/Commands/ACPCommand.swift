...
struct ACPCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        subcommands: [ACPProfilesCommand.self]
    )
...
struct ACPProfilesCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "profiles",
        abstract: "Print discovered ACP agent profiles as JSON."
    )

    @Flag(name: .long, help: "Emit the versioned profile bundle as JSON.")
    var json = false
    ...
    func run() async throws {
        ...
        let profiles = entries.compactMap { entry -> ACPProfile? in
            guard entry.knownProperties["privateTimelineID"] != nil else { return nil }
            return ACPProfile(
        ...
        let bundle = ACPProfileBundle(version: 1, defaultProfile: profiles.count == 1 ? profiles[0].id : nil, profiles: profiles)
