...
/// One disposable Chibi worker process with a run-local restricted environment.
///
...
///
/// The reviewed Chibi build is Linux-only, so this session is implemented for
/// Linux and reports `unsupportedPlatform` elsewhere.
#if os(Linux)
public actor RLMChibiWorkerSession {
    private let configuration: RLMChibiWorkerConfiguration
    private let host: any RLMChibiHost
    ...
    public func start() async throws {
        ...
        guard fileManager.isExecutableFile(atPath: configuration.executablePath) else {
            throw RLMChibiWorkerError.executableMissing(configuration.executablePath)
        }
        guard fileManager.isExecutableFile(atPath: configuration.limitExecutablePath) else {
            throw RLMChibiWorkerError.limitToolMissing(configuration.limitExecutablePath)
        }
        guard fileManager.fileExists(atPath: configuration.workerScriptPath) else {
            throw RLMChibiWorkerError.workerScriptMissing(configuration.workerScriptPath)
        }
        ...
        let process = Process()
        process.executableURL = URL(fileURLWithPath: configuration.limitExecutablePath)
        process.arguments = [
        ...
        do {
            ...
            throw RLMChibiWorkerError.spawnFailed("\(error)")
        }
        ...
        guard case let .ready(frame)? = readyFrame else {
            terminate()
            throw RLMChibiWorkerError.initializationFailed("worker did not report ready")
        }
        guard frame.runID == configuration.runID else {
            terminate()
            throw RLMChibiWorkerError.initializationFailed("worker run identity mismatch")
        }
        ready = frame
    }

    /// Validates and evaluates one cell in the run-local environment.
    public func evaluate(source: String, cellID: Int? = nil) async -> RLMChibiEvaluationOutcome {
    ...
    private func performEvaluation(cellID: Int, generation: UInt64) async -> RLMChibiEvaluationOutcome {
        while true {
            if cancellation.isCancelled {
                return .cancelled
            }
    ...
    private func withWallDeadline(
        _ operation: @escaping @Sendable () async -> RLMChibiEvaluationOutcome
    ) async -> RLMChibiEvaluationOutcome {
        await withTaskGroup(of: RLMChibiEvaluationOutcome.self) { group in
    ...
    private final class FailureBox: @unchecked Sendable { // SAFETY: the NSLock serializes the single reader-thread write against actor reads.
        ...
        private var value: RLMChibiEvaluationOutcome?

        func set(_ outcome: RLMChibiEvaluationOutcome) {
            lock.lock()
            value = outcome
            lock.unlock()
        }

        func get() -> RLMChibiEvaluationOutcome? {
            lock.lock()
            defer { lock.unlock() }
            return value
        }
...
public actor RLMChibiWorkerSession {
    ...
    public func start() async throws {
        throw RLMChibiWorkerError.unsupportedPlatform
    }
