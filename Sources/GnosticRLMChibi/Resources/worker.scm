...
(define (string-split value delimiter)
  (if (not (string? value))
      (gnostic-raise "string-split value must be a string"))
  (if (not (string? delimiter))
      (gnostic-raise "string-split delimiter must be a string"))
  (let ((value-length (string-length value))
        (delimiter-length (string-length delimiter)))
    (if (= delimiter-length 0)
        (gnostic-raise "string-split delimiter must not be empty"))
    (let ((matches-at?
           (lambda (index)
             (let loop ((offset 0))
...
(define (sort-list values less?)
  ...
  (define (halve values)
    (let loop ((rest values)
               (remaining (quotient (length values) 2))
               (left '()))
      (if (= remaining 0)
          (list (reverse left) rest)
          (loop (cdr rest) (- remaining 1) (cons (car rest) left)))))
  (if (or (null? values) (null? (cdr values)))
      values
      (let ((halves (halve values)))
        (merge (sort-list (car halves) less?) (sort-list (cadr halves) less?)))))

(define (sort values less?)
  (let ((vector-input? (vector? values)))
    (let ((items (if vector-input? (vector->list values) values)))
      (if (not (list? items))
          (gnostic-raise "sort expects a list or vector"))
