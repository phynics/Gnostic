...
.PHONY: help image require-package resolve worktree-bootstrap build test benchmark scenario-stage0 docs-check lint harness-test runner-smoke acp-smoke container-smoke macos-rlm-smoke verify shell clean dev-up dev-status dev-down sbom

help:
	@echo "Targets: image require-package resolve worktree-bootstrap build test benchmark scenario-stage0 docs-check lint harness-test runner-smoke acp-smoke container-smoke macos-rlm-smoke verify shell clean dev-up dev-status dev-down sbom"
...
acp-smoke: require-package image
	@BUILD_DIR="$(BUILD_DIR)" BUILD_LOCK="$(BUILD_LOCK)" SPM_CACHE_DIR="$(SPM_CACHE_DIR)" EXTRA_CONTAINER_MOUNTS="$(EXTRA_CONTAINER_MOUNTS)" IMAGE="$(IMAGE)" CONTAINER_RUNTIME="$(CONTAINER_RUNTIME)" ./.devcontainer/run.sh bash -o pipefail -c 'pgrep mosquitto >/dev/null 2>&1 || mosquitto -c /etc/mosquitto/gnostic.conf -d; bin=$$(swift build $(SWIFT_LOCKED_ARGS) $(SWIFT_WARNING_ARGS) --show-bin-path)/gnostic; test -x "$$bin" || { echo "Could not locate built gnostic executable at $$bin" >&2; exit 1; }; npm ci --prefix Tests/Fixtures/OfficialACPClient --cache .testing/npm-cache; npm ci --legacy-peer-deps --prefix Tests/Fixtures/PiACPClient --cache .testing/npm-cache; GNOSTIC_ACP_BINARY="$$bin" GNOSTIC_ACP_OFFICIAL_CLIENT=/workspace/Tests/Fixtures/OfficialACPClient/lifecycle.mjs GNOSTIC_PI_ACP_CLIENT_FIXTURE=/workspace/Tests/Fixtures/PiACPClient/lifecycle.mjs swift test $(SWIFT_LOCKED_ARGS) $(SWIFT_WARNING_ARGS) --filter GnosticCLITests.ACPSubprocessTests | tee .testing/acp-smoke.log && grep -F "Test run with 5 tests" .testing/acp-smoke.log'
