# RLM scenario manifest — DRAFT for #354

Status: **PRE-AMENDMENT DRAFT.** Written against the promotion gate as stated in
ADR 0012 (`Documentation/Architecture/ADRs/0012-rlm-runtime-selection.md`,
`main` at `faeb16b`). The dual-executor amendment owned by #350 has not landed.
Exactly one slot in this manifest is left open for it, marked **[OPEN: #350]**.
Everything else derives from the gate, which has not moved.

Author: consultant session 3ed79195 (session ended; preserved by coordinator f9746017).
Version: **v5**. Revised after four rounds of coordinator review (session f9746017).
...
## 2. Executor matrix **[OPEN: #350]**

The one slot the amendment parameterises. Pre-amendment, ADR 0012 expects a
single promotion. Post-amendment, the matrix is:
...
| E | bounded retrieval | n/a | unavailable — no implementation exists |
...
When #350 lands, this section is the only one that should need a diff.
