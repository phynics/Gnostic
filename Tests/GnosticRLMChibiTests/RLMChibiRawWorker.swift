...
/// A raw, unvalidated framed client for the Chibi worker.
///
/// It deliberately bypasses `RLMChibiWorkerSession` so tests can prove that the
/// restricted environment itself, not only the parent validator, rejects
/// dangerous forms.
enum RLMChibiRawWorkerError: Error {
    case unavailable
...
final class RLMChibiRawWorker: @unchecked Sendable { // SAFETY: a test drives one helper at a time; the only cross-thread access is the single bounded background frame read.
    ...
    init(runID: String) throws {
        ...
        let process = Process()
        process.executableURL = URL(fileURLWithPath: chibi)
        process.arguments = [
            RLMChibiTestSupport.workerScriptPath,
        ...
        do {
            ...
            throw RLMChibiRawWorkerError.spawnFailed("\(error)")
        }
    ...
    func receive(timeout: TimeInterval = 10) throws -> RLMSchemeWorkerFrame {
        ...
        guard semaphore.wait(timeout: .now() + timeout) == .success else {
            killForTest()
            throw RLMChibiRawWorkerError.closed
        }
